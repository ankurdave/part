object CtrieBenchmark {
  ...
  def benchmarkCtrie[@specialized(Int) ValType](
    ...
    // val snapshot = ctrie.readOnlySnapshot()
    ...
    //   }
    // })
    // var concurrentReadTime = 0L
    // val readerThread = new Thread(new Runnable {
    //   def run(): Unit = {
    //     concurrentReadTime = time {
    //       (0 until T).foreach { i => readTotal += valExtractor(snapshot.getOrElse(randomKeys(i), randomVal)) }
    //     }
    //     readerDone = true
    //   }
    // })
    ...
    // writerThread.join()
    // readerThread.join()
    // println(s"{'measurement': 'concurrent read', 'datastructure': 'ctrie', " +
    //   s"'y': ${T / (concurrentReadTime / 1e9)}, 'valsize': $valSize},")
